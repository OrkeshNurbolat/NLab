...
class RTP(QtWidgets.QWidget):
  ...
  def xcbOnChanged(self, text):
    if(self.mode=='P') : 
      self.P.ix = self.P.CN.index(text) ;
      self.P.xname = self.P.CN[self.P.ix] ;
    elif(self.mode=='T') : 
      self.T.ix = self.T.CN.index(text) ;
      self.T.xname = self.T.CN[self.P.ix] ;
    self.updatePlot(True);
  
  def ycbOnChanged(self, text):
    if(self.mode=='P') : 
      self.P.iy = self.P.CN.index(text) ;
      self.P.yname = self.P.CN[self.P.iy] ;
    elif(self.mode=='T') : 
      self.T.iy = self.T.CN.index(text) ;
      self.T.yname = self.T.CN[self.P.iy] ;
    self.updatePlot(True);
  ...
  def checkLast(self):
    W = None ;
    if(self.mode=='P'):W=self.P ;
    elif(self.mode=='T'):W=self.T ;
    
    # still select the last
    if(W!=None):
      if(W.xname in W.CN):
        W.ix = W.CN.index(W.xname);
        self.xcb.setCurrentIndex(W.ix);

      if(W.yname in W.CN):
        W.iy = W.CN.index(W.yname);
        self.ycb.setCurrentIndex(W.iy);
