...
class XYFMR:
  def __init__(self , xname_ , yname_ , fx_ , fy_ , mode_ , XR  , YR ):
    self.mode = str(mode_); #P fore points , T for traces 
    self.xname = str(xname_);
    self.yname = str(yname_);
    self.fx = str(fx_);
    self.fy = str(fy_);

  def sig(self):
    return hash( (self.xname , self.yname , self.fx, self.fy , self.mode )) ;
...
class RTP(QtWidgets.QWidget):
  ...
  def initGui(self):
    ...
    self.ang2_lb = QtWidgets.QLabel("Rotate Y : ") ;
    self.ang2_lb.setAlignment(QtCore.Qt.AlignRight);
    self.ang2 = QtWidgets.QSlider(Qt.Horizontal);
    self.ang2.setMinimum(0) ;
    self.ang2.setMaximum(3600) ;
    self.ang2.valueChanged.connect(self.angOnChanged) ;
    ...
    self.layout.addWidget(self.ang2_lb  , 1 , 10 );
    self.layout.addWidget(self.ang2     , 1 , 11 );
    self.layout.addWidget(self.plt , 2 , 0 , 1 , 12);
